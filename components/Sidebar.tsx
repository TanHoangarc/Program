...
export const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  const MenuItem = ({ 
    active, onClick, icon: Icon, label, statusColor, hasSubmenu = false, isOpen = false
  }: any) => (
    <button
      ...
      className={`w-full flex items-center justify-between px-4 py-2.5 mb-1 rounded-xl transition-all duration-200 group relative ${
        active ? 'bg-white/20 text-white shadow-lg backdrop-blur-sm border border-white/10' : 'text-slate-300 hover:bg-white/10 hover:text-white'
      }`}
    >
      <div className="flex items-center space-x-3">
        {statusColor ? <div className={`w-2 h-2 rounded-full ${statusColor}`}></div> : <Icon className={`w-5 h-5 ${active ? 'text-teal-300' : 'text-slate-400 group-hover:text-teal-200'}`} />}
        <span className={`text-sm ${active ? 'font-semibold tracking-wide' : 'font-medium'}`}>{label}</span>
      </div>
      {hasSubmenu ? (isOpen ? <ChevronDown className="w-4 h-4 text-slate-400" /> : <ChevronRight className="w-4 h-4 text-slate-400" />) : (active && <div className="w-1.5 h-1.5 rounded-full bg-teal-400 shadow-[0_0_8px_#2dd4bf]"></div>)}
    </button>
  );

  const SubMenuItem = ({ active, onClick, icon: Icon, label }: any) => (
      <button onClick={(e) => { e.preventDefault(); onClick(); }} className={`w-full flex items-center space-x-3 px-4 py-2 rounded-lg text-sm transition-colors mb-1 ${active ? 'text-teal-300 bg-white/5 font-medium' : 'text-slate-400 hover:text-slate-200 hover:bg-white/5'}`}>
  ...
  return (
    <>
      ...
      <div className={`fixed z-50 flex flex-col bg-slate-900 shadow-2xl transition-transform duration-300 ease-in-out w-64 h-full top-0 left-0 md:h-[96vh] md:top-[2vh] md:left-4 md:rounded-3xl ${isOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        ...
        <div className="relative z-10 px-6 py-6 border-b border-white/5 flex justify-between items-center">
           <div className="flex items-center space-x-3 text-white">
              <div className="p-2 bg-gradient-to-tr from-teal-400 to-blue-500 rounded-lg shadow-lg"><Ship className="w-6 h-6 text-white" /></div>
              <div>
        ...
        <nav className="relative z-10 flex-1 px-4 space-y-1 pt-4 overflow-y-auto custom-scrollbar pb-2">
          {canViewOverview && (
            <>
              ...
              <MenuItem active={currentPage === 'salary'} onClick={() => handleNavigate('salary')} icon={Coins} label="Lương" />
            </>
          )}
          {canViewOperations && (
            <>
              ...
              <div>
                 <MenuItem active={['deposit-line', 'deposit-customer'].includes(currentPage)} onClick={() => toggleGroup('deposit')} icon={ArrowRightLeft} label="Quản lý Cược" statusColor="bg-purple-400" hasSubmenu={true} isOpen={openGroups.deposit} />
                {openGroups.deposit && (
                  <div className="pl-6 pr-2 py-2 bg-black/20 rounded-xl mb-2">
          ...
          {canViewAccounting && (
            <div>
                <MenuItem active={['amis-thu', 'amis-chi', 'amis-ban', 'amis-mua'].includes(currentPage)} onClick={() => toggleGroup('amis')} icon={FileUp} label="Kế Toán AMIS" hasSubmenu={true} isOpen={openGroups.amis} />
                {openGroups.amis && (
                  <div className="pl-6 pr-2 py-2 bg-black/20 rounded-xl mb-2">
          ...
          {canViewRecon && <MenuItem active={currentPage === 'reconciliation'} onClick={() => handleNavigate('reconciliation')} icon={Scale} label="Đối Chiếu" />}
          {canViewData && (
            <div>
              <MenuItem active={['data-lines', 'data-customers'].includes(currentPage)} onClick={() => toggleGroup('data')} icon={Database} label="Danh Mục" hasSubmenu={true} isOpen={openGroups.data} />
              {openGroups.data && (
                  <div className="pl-6 pr-2 py-2 bg-black/20 rounded-xl mb-2">
          ...
          {canViewSystem && <MenuItem active={currentPage === 'system'} onClick={() => handleNavigate('system')} icon={Settings} label="Hệ Thống" />}
        </nav>
        ...
        <div className="relative z-10 p-4 mt-auto border-t border-white/5 bg-black/20 space-y-3">
          ...
          {canSendPending && (
            <button onClick={(e) => { e.preventDefault(); onSendPending(); }} className="w-full bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold py-2.5 px-3 rounded-xl transition-all shadow-lg flex items-center justify-center space-x-2">
          ...
          {currentUser && (
            <div className="flex items-center justify-between px-1">
               <div className="flex items-center space-x-3">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-teal-500 to-blue-500 flex items-center justify-center shadow-lg border border-white/10"><span className="text-xs font-bold text-white">{(currentUser.username || '?').charAt(0).toUpperCase()}</span></div>
               ...
               <button onClick={(e) => { e.preventDefault(); onLogout(); }} className="p-2 text-slate-400 hover:text-white hover:bg-white/10 rounded-lg transition-all" title="Đăng xuất"><LogOut className="w-4 h-4" /></button>
            </div>
          )}
        </div>
      </div>
