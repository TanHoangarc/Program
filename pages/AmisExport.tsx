  ...
  const executeExport = async (startVoucher?: string) => {
    ...
    if (startVoucher) {
        ...
        if (startMatch) {
            ...
            uniqueDocNos.sort((a: any, b: any) => {
                const matchA = typeof a === 'string' ? a.match(/^([A-Za-z\-_]*)(\d+)$/) : null;
                const matchB = typeof b === 'string' ? b.match(/^([A-Za-z\-_]*)(\d+)$/) : null;
                const numA = matchA ? parseInt(matchA[2], 10) : 0;
                const numB = matchB ? parseInt(matchB[2], 10) : 0;
                return numA - numB;
            });

            uniqueDocNos.forEach((origDocNo: any, idx) => {
