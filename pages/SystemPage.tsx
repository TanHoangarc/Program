            ...
            {pendingRequests.length > 0 ? (
                <div className="space-y-4">
                    {pendingRequests.map((req) => {
                        // Safety checks to prevent rendering crashes
                        const reqId = req.id || Math.random().toString();
                        const userName = req.user || 'Unknown User';
                        const userChar = userName.charAt(0).toUpperCase();
                        const timestamp = req.timestamp ? new Date(req.timestamp).toLocaleString('vi-VN') : 'N/A';
                        const jobCount = Array.isArray(req.jobs) ? req.jobs.length : 0;
                        const custCount = Array.isArray(req.customers) ? req.customers.length : 0;
                        const lineCount = Array.isArray(req.lines) ? req.lines.length : 0;
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div>
                                    ...
                                    <div className="text-xs text-slate-500 mt-1 flex gap-3">
                                        <span>Jobs: <strong>{jobCount}</strong></span>
                                        <span>|</span>
