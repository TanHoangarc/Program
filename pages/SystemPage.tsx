...
export const SystemPage: React.FC<SystemPageProps> = ({ 
  ...
  const handleApprove = (req: any) => {
      ...
      if (window.confirm(`Duyệt dữ liệu từ ${userName}?\n(Dữ liệu sẽ được gộp vào hệ thống)`)) {
            ...
            {pendingRequests.length > 0 ? (
                <div className="space-y-4">
                    {pendingRequests.map((req, index) => {
                        // Safe rendering logic to avoid crashes if data is missing
                        const reqId = req.id || `req-${index}`;
                        const userName = req.user || 'Unknown';
                        const userInitial = userName.charAt(0).toUpperCase();
                        const timeStr = req.timestamp ? new Date(req.timestamp).toLocaleString('vi-VN') : 'N/A';
                        
                        const jobsCount = Array.isArray(req.jobs) ? req.jobs.length : 0;
                        const custCount = Array.isArray(req.customers) ? req.customers.length : 0;
                        const linesCount = Array.isArray(req.lines) ? req.lines.length : 0;

                        return (
                            <div key={reqId} className="bg-white/60 p-4 rounded-xl border border-white/50 shadow-sm flex flex-col md:flex-row justify-between items-center gap-4 transition-all hover:shadow-md">
                                <div className="flex items-center space-x-4">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-xs text-slate-500 mt-1 flex gap-3">
                                            <span>Jobs: <strong>{jobsCount}</strong></span>
                                            <span>|</span>
                                            <span>Customers: <strong>{custCount}</strong></span>
                                            <span>|</span>
                                            <span>Lines: <strong>{linesCount}</strong></span>
                                        </div>
                                    </div>
                                </div>
...
<change>
<file>components/BookingDetailModal.tsx</file>
<description>Update file upload logic to use absolute server path and improved error handling</description>
<content><![CDATA[
...
export const BookingDetailModal: React.FC<BookingDetailModalProps> = ({ booking, onClose, onSave, zIndex = 'z-50' }) => {
  ...
  const handleUploadFile = async () => {
    ...
    try {
      ...
      const folderName = `${yearShort}.${month}`; // e.g., "25.07"
      
      // ABSOLUTE PATH for Server
      // Note: We use double backslashes for JS string escaping
      const folderPath = `E:\\ServerData\\Invoice\\${folderName}`;

      // 2. Generate New Filename: Line.Booking.Invoice.dd.mm.yyyy.ext
      const originalName = selectedFile.name;
      const extension = originalName.substring(originalName.lastIndexOf('.'));
      
      // Get safe strings for filename (remove special chars)
      ...
      const dateFileStr = `${day}.${month}.${yearFull}`;
      ...
      formData.append('folderPath', folderPath); // Sending Absolute Path to Server
      ...
      if (res.ok) {
        alert(`Đã lưu file thành công!\n\nĐường dẫn: ${folderPath}\\${newFileName}`);
        setSelectedFile(null);
        if (fileInputRef.current) fileInputRef.current.value = '';
      } else {
        const errorText = await res.text();
        console.error("Upload failed details:", errorText);
        alert(`Lỗi khi tải file lên server (${res.status}): ${errorText || 'Vui lòng kiểm tra quyền truy cập ổ E hoặc kết nối mạng.'}`);
      }
    } catch (err) {
      console.error("Upload failed", err);
      alert("Không thể kết nối với máy chủ. Vui lòng kiểm tra lại đường truyền.");
    } finally {
